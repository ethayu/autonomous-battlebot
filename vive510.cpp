...
// need global interrupt Arduino attachInterrupt won't take member function as
// interrupt need to pass THIS as arg since all vive object interrupts call same
// global interrupt
...
Vive510::Vive510(int pin) { m_pin = pin; }
...
uint16_t Vive510::xCoord() { return m_xCoord; }

uint16_t Vive510::yCoord() { return m_yCoord; }

int Vive510::isKPulse(uint32_t pulsewidth) {
  if (pulsewidth < 75 || (pulsewidth > 85 && pulsewidth < 95) ||
...
// move checkflag to be backwards checking...think about whether tu link
// w/spuriuos
...
#ifdef DEBUG
      ets_printf("Pin%d:%d  r=%d", m_pin, pulsewidth,
                 m_usRising - m_lastFalling);
#endif
...
void Vive510::stop() { detachInterrupt(digitalPinToInterrupt(m_pin)); }

int Vive510::status() { return m_vivestatus; }
